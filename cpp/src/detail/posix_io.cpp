                                      ...
                                      std::size_t devPtr_offset)
{
  ...
  while (bytes_remaining > 0) {
    ...
    std::size_t useful         = std::min(bytes_remaining, bounce_buffer_size - prefix);
    ...
    ssize_t nbytes_read = posix_host_io<IOOperationType::READ, PartialIO::YES>(
      -1, bounce_buffer.get(), aligned_size, convert_size2off(aligned_offset), fd_direct_on);
    KVIKIO_EXPECT(nbytes_read > static_cast<ssize_t>(prefix),
                  "pread(O_DIRECT): unexpected EOF within the requested range");

    std::size_t actual_useful = std::min(useful, static_cast<std::size_t>(nbytes_read) - prefix);
